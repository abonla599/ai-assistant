...
> **2026-09-23 复查补记（不改上面那条决定，只是修正它对实现的描述）**：v0.18 落地后代码复查发现
> "不推进排期"这句只写对了 `once`（本来就该消失，跟正常支一样），却把 `daily`/`weekly` 也一起
> 写成了"停在原地不动"——闹钟是一次性的，停在原地不等于"之后还会再来"，那样只会变成重启也不
> 重排的永久孤儿。现在两支共用同一套"记账 → advance → 重排"骨架，唯一区别是跳过 `notify()`；
> `once` 走这条骨架的结果跟以前一样是"消失"，只是不再被后面每一次广播重复记一笔 `missed`。
> 同一批复查还补上了 `PermissionStatus.notificationsGranted` 对通知总开关（`areNotificationsEnabled`）
> 的判定——只查 `checkSelfPermission` 在 Android 12 及以下恒真、且查不到用户后来关掉的总开关。
> 这两处改动的锁见 `test_android_shell.py`。
