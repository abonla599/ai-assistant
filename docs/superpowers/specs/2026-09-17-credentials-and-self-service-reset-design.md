...
| 5 | 重置成功要不要计入预算 | **计**：每来源每 24 小时最多 3 次成功重置（`MAX_RESETS_PER_SOURCE` / 独立账本 `_RESETS`）；**猜答案的失败也独立成册**（`_RESET_FAILS`，任何成功路径都不清它） | 猜密保答案比猜口令容易得多；只罚失败不罚成功，等于允许"慢慢试到成功为止"。而失败账若与登录/注册共用一本，猜的人随手登录成功一次就把预算还给自己了——两本都要拆，见 §4.5 |
...
- 成功仍返回 `{token, user_id, username, role}`，注册即登录；同时记一格 `_REGISTERS`、
  并 `_FAILS.pop(ip)` 清掉该来源的失败账（这一条是登录/注册的既有契约，找回没有它，见 §4.5）。
...
| 账本 | 窗口 / 上限 | 记什么 | 谁会清它 |
| --- | --- | --- | --- |
| `_FAILS` | 600 秒 / 10 次 | 登录失败 + 注册撞名 | 该来源一次**成功**登录或注册（`_FAILS.pop(ip)`） |
...
- `/v1/auth/reset` 只看 `_RESET_FAILS` 与 `_RESETS` 这两本，`_FAILS` 它**既不看不写**。
  为什么猜答案的失败必须单独一册：`_FAILS` 会被一次成功登录清空，而一个正在猜三题的人
  随手就能拿到一次那样的成功（猜对自己的口令、或再注册一个小号）——共用一本等于每成功一次
  把预算还给他。判据：`test_no_other_success_pays_off_the_guessing_ledger`、
...
3. **两本失败账拆开之后，同一来源每个 10 分钟窗口能烧的猜测次数翻倍**：`_FAILS` 10 次
   ...
   独立了，撞名枚举回到"只有 `_FAILS` 那一本在挡"。这是拆账的代价，换来的是"猜答案的人
   不能靠随手登录成功一次把预算还给自己"（§4.5）——两害相权要的是后者。
...
6. **老账户没有 `answer_hashes`，因此永远走不通自助重置**。它们不会被回填（§7），
   ...
   直到人在 `/admin` 把它们删掉。
7. **422 现在全站只有同一句中文**（`app/main.py:on_malformed_body`）。好处是响应形状单一
   来源在服务端，两个客户端（`api.js`、`admin.js`）都不必各自兜一层，`curl` 与桌面壳
   也看得见同一句话。代价要直说：**非 auth 端点的字段名不再回给客户端**（例如
   `/v1/memory/search` 的 `top_k` 越界、会话消息里的非对象项），排障时只能看服务端日志里
   那行 `⚠️ 请求体形状不对（body.messages.0）`。目前 `api.js` 自己夹了 `top_k`，没有现存
   需求被这条挡掉；但哪天有客户端要按字段做分支，这里得再议。

结论：对"发给朋友用的小工具"这个定位，1–5 与 7 是认了的代价；第 6 条不是取舍，是
...
| 4 | 猜答案的失败与登录/注册共用 `_FAILS` | 独立一本 `_RESET_FAILS`（同窗口、同上界），且**任何成功路径都不清它** | `_FAILS` 的契约是"该来源成功登录/注册一次就把失败账清零"。猜三题的人随手就能拿到一次那样的成功（猜对自己的口令、再注册一个小号），于是每 10 次里蒙中一次他就永远限不住——**账本洗白**。代价见 §6 第 3 条 |
...
| 9 | 422 由各客户端各自兜一层（设想里没有这一条） | 服务端一个 handler 把结构化 detail 压成一句中文，字段路径只进服务端日志 | 形状有 N 个事实来源时，`curl` 与桌面壳这两个没兜的照样看不懂（`new Error(detail)` 会显示成 `[object Object]`）。代价：非 auth 端点的字段名不回客户端，见 §6 第 7 条 |
