...
| 7 | 三题是"全对"还是"对一题即可" | **必须全对**（`all(results)`） | 只要对一题就放行，这条路等于敞着的门：三格里有一格（换题前的"你的用户名是什么？"）读得到用户名就 100% 答得对。全对之下，弱的那一格只是不再贡献熵，不会替攻击者开门 |
...
- **问题文本不进库**。三题是全站常量 `app/core/auth.py:RECOVERY_QUESTIONS`
  （`("你的手机号后四位是什么？", "你小学在哪上？", "你父母姓氏的拼音首字母各一个是什么？")`），
...
1. **找回答案是可猜、可社工的凭据**，熵通常远低于 8 位以上口令。三题的文本现在是全站
   ...
   所以**答案的实际熵就是那三句话的熵**。题面定成"手机号后四位 / 小学 / 父母姓氏首字母"
   之后，手册里那三条作答指引就成了安全性的一部分（全称小学名、公开场合出现过的姓、
   顺手写成拼音全拼，都会把这一格的熵打回原形）。
2. **这三格各自的天花板**（09-18 换题后的实测口径）：`手机号后四位` 上限 10^4，且
   **换号即锁死**——目前界面没有"换答案"入口（存储层有这条通路，见决定 2 与 §10）；
   `父母姓氏的拼音首字母各一个` 上限 26×26 = 676，而姓氏恰恰是社交场合最容易拿到的信息；
   `小学` 取决于人填全称还是填自己叫的名字。整条路没塌靠两层叠加：决定 7 的"三题必须全对"
   （要同时中三格），以及 §4.5 那四本账——在线每猜一次要付三枚 bcrypt（实测约 0.57s）、
   每来源每 10 分钟只有 10 次，这个组合空间在线打不完。**真正的前提是离线那一层**：
   谁拿到 `users.json`，bcrypt 就是唯一屏障，而 676 × 10^4 × 小学数 这种量级对离线爆破
   不算大——所以那台机器的文件本身必须当秘密看（`.env` 曾被跟踪泄露 5 个月的前科见 §7）。
...
- **界面没有"改找回答案"的入口**：`reset_password` 支持带 `new_answers`（有测试钉着），
  但找回那屏只让改密码。于是"换手机号"这件事会把一个人永久锁在自助重置之外——
  要么补那一格，要么在手册里把人推到管理员面前（现在写的是后者）。
